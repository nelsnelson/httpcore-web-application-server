...
import org.nelsnelson.org.apache.commons.httpclient.util.ParameterFormatter;
...
public class RFC2109Spec extends CookieSpecBase {
    ...
    private void formatCookieAsVer(final StringBuffer buffer, final Cookie cookie, int version) {
        ...
        formatParam(buffer, new BasicNameValuePair(cookie.getName(), value), version);
        if ((cookie.getPath() != null) && cookie.isPathAttributeSpecified()) {
          buffer.append("; ");
          formatParam(buffer, new BasicNameValuePair("$Path", cookie.getPath()), version);
        }
        if ((cookie.getDomain() != null) 
            && cookie.isDomainAttributeSpecified()) {
            buffer.append("; ");
            formatParam(buffer, new BasicNameValuePair("$Domain", cookie.getDomain()), version);
        }
    }
    ...
    public String formatCookie(Cookie cookie) {
        ...
        formatParam(buffer, 
                new BasicNameValuePair("$Version", Integer.toString(version)), 
                version);
        buffer.append("; ");
