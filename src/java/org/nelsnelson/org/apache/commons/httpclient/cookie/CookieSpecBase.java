...
import org.nelsnelson.org.apache.commons.httpclient.util.DateParseException;
import org.nelsnelson.org.apache.commons.httpclient.util.DateUtil;
...
public class CookieSpecBase implements CookieSpec {
    ...
    public Cookie[] parse(String host, int port, String path, 
        ...
        if (isNetscapeCookie && header.indexOf("=") >= 0) {
            ...
            headerElements = new HeaderElement[] {
                new BasicHeaderElement(name, value)
            };
        } else {
            ...
            try {
                bufferedHeader = new BufferedHeader(buffer);
                headerElements = getElements(bufferedHeader);
            }
    ...
    public HeaderElement[] getElements(BufferedHeader bufferedHeader) throws ParseException {
        final ParserCursor cursor = new ParserCursor(0, bufferedHeader.getBuffer().length());
        cursor.updatePos(bufferedHeader.getValuePos());
        return BasicHeaderValueParser.INSTANCE.parseElements(bufferedHeader.getBuffer(), cursor);
    }
    ...
    public Header formatCookieHeader(Cookie[] cookies) {
        ...
        return new BasicHeader("Cookie", formatCookies(cookies));
    }
    ...
    public Header formatCookieHeader(Cookie cookie) {
        //LOG.trace("enter CookieSpecBase.formatCookieHeader(Cookie)");
        return new BasicHeader("Cookie", formatCookie(cookie));
    }
